...
// CompeticionService defines the interface for the competition service
type CompeticionService interface {
	GetRondaCompeticionSumo() (*models.Ronda, error)
	GetRondaCompeticionSigueLineas() (*models.RondaSigueLineas, error)
	StartCompetitionByID(id int) error
	StartCompetitionSumo(id uint) error
	StartCompetitionSigueLineas(id uint) error
	SetWinnerSumo(req vo.RequestBody) error
}

// competicionServiceImpl is the implementation of CompeticionService
type competicionServiceImpl struct {
	db *gorm.DB
}

// NewCompeticionService creates a new instance of CompeticionService
func NewCompeticionService(db *gorm.DB) CompeticionService {
	return &competicionServiceImpl{db: db}
}

func (s *competicionServiceImpl) GetRondaCompeticionSumo() (*models.Ronda, error) {
	var ronda models.Ronda
	err := s.db.Table("rondas").Where("fecha_hora_competion is null").First(&ronda).Error
	if err != nil {
	...
	s.db.Model(&ronda).Update("fecha_hora_competion", time.Now())
	...
	s.db.First(ronda.RobotA, "ID = ?", ronda.RobotAID)
	s.db.First(ronda.RobotB, "ID = ?", ronda.RobotBID)
	return &ronda, nil
}

func (s *competicionServiceImpl) GetRondaCompeticionSigueLineas() (*models.RondaSigueLineas, error) {
	var ronda models.RondaSigueLineas
	err := s.db.Table("ronda_sigue_lineas").Where("fecha_hora_competion is null").First(&ronda).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	s.db.First(ronda.Robot, "ID = ?", ronda.RobotID)
	//TODO falta el arbitro
	fmt.Printf("Ronda tomada por arbitro id:%d ronda:[%v]\n", -1, ronda)
	return &ronda, nil
}

func (s *competicionServiceImpl) StartCompetitionByID(id int) error {
	//Obtenemos la categoria por ID
	var categoria models.Categoria
	result := s.db.First(&categoria, id)
	if result.RowsAffected == 0 {
		return fmt.Errorf("failed to check last start time: %w", errors.New("Id not found"))
	}
	if id == 3 {
		err := s.StartCompetitionSigueLineas(categoria.ID)
		if err != nil {
			return fmt.Errorf("failed to start competition: %w", err)
		}
	} else if id == 1 {
		err := s.StartCompetitionSumo(categoria.ID)
		if err != nil {
			return fmt.Errorf("failed to start competition: %w", err)
		}
	} else {
...
func (s *competicionServiceImpl) StartCompetitionSumo(id uint) error {
	// Verificar si ya hay una ronda en los últimos 3 meses
	var ronda models.Ronda
	err := s.db.Table("rondas").
	...
	s.db.Where("categoria_id = ?", id).Find(&robots)
	if len(robots) == 0 {
	...
	for i := 0; i < len(robots); i += 2 {
		...
		if err := s.db.Create(&rondaNueva).Error; err != nil {
...
func (s *competicionServiceImpl) StartCompetitionSigueLineas(id uint) error {
	fmt.Println("Moviendo robots a la tabla de competicion sigue lineas")
	// Verificar si ya hay una ronda en los últimos 3 meses
	var ronda models.RondaSigueLineas
	err := s.db.Table("ronda_sigue_lineas").
		Select("MAX(fecha_hora_insercion) AS fecha_hora_insercion").
		Scan(&ronda).Error
	if err != nil {
		return fmt.Errorf("error al obtener la última ronda: %w", err)
...
func (s *competicionServiceImpl) SetWinnerSumo(req vo.RequestBody) error {
	db := s.db.Debug()
	var ronda models.Ronda
